...
type Cut = 'cushion' | 'oval' | 'marquise' | 'round';
...
export function Gem({ color = '#E8A1B8', cut = 'cushion', size = 220 }: { color?: string; cut?: Cut; size?: number }) {
  ...
  if (cut === 'cushion') {
    body = (
      <g>
        <path d="M30 80 Q30 30 80 30 L140 30 Q190 30 190 80 L190 140 Q190 190 140 190 L80 190 Q30 190 30 140 Z" fill={`url(#${id}-bg)`} stroke={dark} strokeWidth="1" />
    ...
    body = (
      <g>
        <path d="M110 15 Q190 110 110 205 Q30 110 110 15 Z" fill={`url(#${id}-bg)`} stroke={dark} strokeWidth="1" />
  ...
  const wrapStyle: CSSProperties & { ['--gem-glow']?: string } = {
    width: size,
    height: size,
    ['--gem-glow']: `${color}88`,
  };
  ...
  return (
    <div className="jg-gem-wrap" style={wrapStyle}>
